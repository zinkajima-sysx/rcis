      ...
      <div className="bg-slate-800 rounded-2xl border border-white/10 flex flex-col flex-1 overflow-hidden">
        <div className="p-4 border-b border-white/10 bg-slate-800/80 shrink-0">
          <div className="relative max-w-sm w-full">
            ...
            <input type="text" placeholder="Cari Daop / Divre..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} className="w-full bg-slate-900 border border-white/10 rounded-lg pl-10 pr-4 py-2 text-sm text-slate-200 focus:outline-none focus:border-sky-500 " />
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap">
            <thead>
              <tr className="border-b border-white/10 bg-slate-800/50">
                ...
                <th className="py-4 px-5 text-xs text-slate-400 uppercase tracking-wider font-semibold">Nama Daop / Divre</th>
          ...
          <form onSubmit={handleSave} className="bg-slate-800 border border-white/10 w-full max-w-md rounded-2xl shadow-2xl flex flex-col">
            ...
            <div className="p-6 bg-slate-900/50 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-medium text-slate-300">Nama Daop / Divre <span className="text-rose-400">*</span></label>
                <input required type="text" value={formData.nama} onChange={(event) => setFormData({ ...formData, nama: event.target.value })} placeholder="Contoh: Daop 1 Jakarta" className="w-full bg-slate-900 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-slate-200 focus:outline-none focus:border-sky-500 " />
              </div>
            </div>
