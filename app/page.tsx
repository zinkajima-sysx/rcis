  ...
  return (
    <main className="flex-1 flex flex-col p-8 gap-6 overflow-y-auto no-scrollbar w-full">
      <header className="flex justify-between items-center shrink-0 gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-slate-50">Dashboard RCIS</h1>
          <p className="text-slate-400 text-sm mt-1">Ringkasan aplikasi berdasarkan data real yang tersimpan di database.</p>
        </div>
        <div className="flex items-center gap-3 flex-wrap justify-end">
          ...
          <div className="text-right">
            <div className="text-sm font-semibold text-slate-100">{currentUser?.nama ?? 'Pengguna RCIS'}</div>
      ...
      <div>
        <h2 className="text-sm font-semibold text-slate-300 uppercase tracking-wider mb-3">Ringkasan Inventaris</h2>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 shrink-0">
          <div className="bg-slate-800 p-5 rounded-2xl border border-sky-500/20 flex flex-col relative overflow-hidden">
            <div className="text-xs text-sky-400 uppercase tracking-widest mb-1 font-medium">Total Alkes</div>
            <div className="text-3xl font-bold text-slate-50 mt-1">{alkesSummary.length}</div>
            <Stethoscope className="absolute right-4 bottom-4 text-sky-500/10" size={48} />
          </div>
          <div className="bg-slate-800 p-5 rounded-2xl border border-emerald-500/20 flex flex-col relative overflow-hidden">
            <div className="text-xs text-emerald-400 uppercase tracking-widest mb-1 font-medium">Layak Pakai</div>
            <div className="text-3xl font-bold text-slate-50 mt-1">{kondisiStats.layak}</div>
            <Activity className="absolute right-4 bottom-4 text-emerald-500/10" size={48} />
          </div>
          <div className="bg-slate-800 p-5 rounded-2xl border border-amber-500/20 flex flex-col relative overflow-hidden">
            <div className="text-xs text-amber-400 uppercase tracking-widest mb-1 font-medium">Butuh Perbaikan</div>
            <div className="text-3xl font-bold text-slate-50 mt-1">{kondisiStats.perbaikan}</div>
            <PackageCheck className="absolute right-4 bottom-4 text-amber-500/10" size={48} />
          </div>
          <div className="bg-slate-800 p-5 rounded-2xl border border-rose-500/20 flex flex-col relative overflow-hidden">
            <div className="text-xs text-rose-400 uppercase tracking-widest mb-1 font-medium">Rusak / Tidak Layak</div>
            <div className="text-3xl font-bold text-slate-50 mt-1">{kondisiStats.rusak}</div>
            <XOctagon className="absolute right-4 bottom-4 text-rose-500/10" size={48} />
          </div>
        </div>
      </div>

      <div>
        <h2 className="text-sm font-semibold text-slate-300 uppercase tracking-wider mb-3">Statistik Modul</h2>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 shrink-0">
          <div className="bg-slate-800/50 p-4 rounded-xl border border-white/5 flex items-center justify-between">
            <div>
              <div className="text-xs text-slate-400 font-medium">Galeri Tersimpan</div>
              <div className="text-2xl font-bold text-sky-400 mt-1">{totalGaleri}</div>
            </div>
            <ImageIcon className="text-sky-500/40" size={24} />
          </div>
          <div className="bg-slate-800/50 p-4 rounded-xl border border-white/5 flex items-center justify-between">
            <div>
              <div className="text-xs text-slate-400 font-medium">Handover Tersimpan</div>
              <div className="text-2xl font-bold text-emerald-400 mt-1">{totalHandover}</div>
            </div>
            <PackageCheck className="text-emerald-500/40" size={24} />
          </div>
          <div className="bg-slate-800/50 p-4 rounded-xl border border-white/5 flex items-center justify-between">
            <div>
              <div className="text-xs text-slate-400 font-medium">User Aktif Sistem</div>
              <div className="text-2xl font-bold text-violet-400 mt-1">{totalUsers}</div>
            </div>
            <UserRound className="text-violet-500/40" size={24} />
          </div>
          <div className="bg-slate-800/50 p-4 rounded-xl border border-white/5 flex items-center justify-between">
            <div>
              <div className="text-xs text-slate-400 font-medium">Jadwal Selesai</div>
              <div className="text-2xl font-bold text-amber-400 mt-1">{jadwalSelesai}</div>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 shrink-0 mt-2">
        <div className="bg-slate-800 p-5 rounded-2xl border border-white/10 flex flex-col justify-center">
          <div className="text-xs text-slate-400 uppercase tracking-widest mb-1 font-medium">Kegiatan Sedang Berlangsung</div>
          {currentSchedule ? (
            <>
              <div className="text-lg font-semibold text-slate-50 mt-1">{currentSchedule.title}</div>
            ...
            <div className="text-sm text-slate-400 mt-1">Belum ada kegiatan dengan status berlangsung.</div>
          )}
        </div>
        <div className="bg-slate-800 p-5 rounded-2xl border border-white/10 flex flex-col">
          <div className="text-xs text-slate-400 uppercase tracking-widest mb-3 font-medium">Ringkasan Status Jadwal</div>
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="bg-slate-900/50 rounded-xl p-3 border border-white/5">
              <div className="text-xs text-slate-400">Akan Datang</div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 shrink-0">
          ...
          <div className="bg-slate-800/50 p-4 rounded-xl border border-rose-500/20 flex items-center justify-between">
            <div>
              <div className="text-xs text-slate-400 font-medium">Lewat Masa Kalibrasi</div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div className="bg-slate-800 rounded-2xl border border-white/10 flex flex-col overflow-hidden min-h-[280px]">
          <div className="p-5 border-b border-white/10 shrink-0">
            <h2 className="font-semibold text-slate-50 text-base">Jadwal Berikutnya</h2>
          </div>
          <div className="p-5 flex flex-col gap-3">
            {upcomingSchedules.length > 0 ? upcomingSchedules.map((item) => (
              <div key={item.id} className="p-4 rounded-xl bg-slate-900/50 border border-white/5">
                <div className="text-sm font-semibold text-slate-100">{item.title}</div>
                <div className="text-xs text-slate-400 mt-1">{formatDateLabel(item.date)}</div>
        ...
        <div className="bg-slate-800 rounded-2xl border border-white/10 flex flex-col overflow-hidden min-h-[280px]">
          <div className="p-5 border-b border-white/10 shrink-0">
            <h2 className="font-semibold text-slate-50 text-base">Alkes Terbaru</h2>
          </div>
          <div className="overflow-x-auto w-full">
            <table className="w-full text-left border-collapse whitespace-nowrap">
              <thead>
                <tr className="border-b border-white/10 bg-slate-800/50">
                  <th className="py-4 px-5 text-xs text-slate-400 uppercase tracking-wider font-semibold">Kode</th>
                  <th className="py-4 px-5 text-xs text-slate-400 uppercase tracking-wider font-semibold">Nama Alat</th>
                  <th className="py-4 px-5 text-xs text-slate-400 uppercase tracking-wider font-semibold">Kategori</th>
                  <th className="py-4 px-5 text-xs text-slate-400 uppercase tracking-wider font-semibold">Kondisi</th>
                </tr>
              </thead>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div className="bg-slate-800 rounded-2xl border border-white/10 flex flex-col overflow-hidden min-h-[280px]">
          <div className="p-5 border-b border-white/10 shrink-0">
            <h2 className="font-semibold text-slate-50 text-base">Alkes Mendekati Kalibrasi</h2>
          </div>
          <div className="p-5 flex flex-col gap-3">
            {alkesKalibrasiTerdekat.length > 0 ? alkesKalibrasiTerdekat.slice(0, 5).map((item) => (
              <div key={`${item.id}-near`} className="p-4 rounded-xl bg-slate-900/50 border border-amber-500/10">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="text-sm font-semibold text-slate-100">{item.nama}</div>
                ...
                <div className="text-xs text-slate-400 mt-2">Jadwal kalibrasi: {formatDateIndonesia(item.jadwalKalibrasi)}</div>
              </div>
            )) : (
        ...
        <div className="bg-slate-800 rounded-2xl border border-white/10 flex flex-col overflow-hidden min-h-[280px]">
          <div className="p-5 border-b border-white/10 shrink-0">
            <h2 className="font-semibold text-slate-50 text-base">Alkes Lewat Kalibrasi</h2>
          </div>
          <div className="p-5 flex flex-col gap-3">
            {alkesLewatKalibrasi.length > 0 ? alkesLewatKalibrasi.slice(0, 5).map((item) => (
              <div key={`${item.id}-overdue`} className="p-4 rounded-xl bg-slate-900/50 border border-rose-500/10">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="text-sm font-semibold text-slate-100">{item.nama}</div>
                    <div className="text-xs text-sky-400 font-mono mt-1">{item.kode}</div>
